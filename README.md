...
- 自动识别 Codex 宠物浮窗，在宠物左侧以紧凑卡片跟随移动。
- 宠物隐藏时同步隐藏，再次唤醒时自动显示；短暂识别丢失不会导致浮窗闪烁。
...
- Detects the Codex pet window and follows it with a compact card on the pet's left.
- Hides when the pet is tucked away and reappears when it wakes up, while tolerating brief detection misses to prevent flicker.
